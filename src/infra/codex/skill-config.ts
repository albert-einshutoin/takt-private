...
// Keep TAKT discovery bounded to prevent untrusted skill trees from causing
// unbounded startup work before a Codex process is launched.
...
function lstatIfResolvable(targetPath: string): Stats | undefined {
  try {
...
function collectSkillFiles(
  ...
  const rootStat = lstatIfResolvable(root);
  ...
  for (const entry of entries) {
    ...
    const realEntryPath = entry.isSymbolicLink() ? realpathIfResolvable(entryPath) : entryPath;
...
function resolveHomeDirectory(env: Readonly<Record<string, string | undefined>>): string {
  ...
  return resolve(configuredHome || homedir());
}
...
function getUserSkillRoots(
  ...
  return {
    roots: [join(home, '.agents', 'skills'), codexSkillsRoot],
