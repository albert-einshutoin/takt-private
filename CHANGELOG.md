...
- Copy workspace isolation (#731). Tasks can now use `isolation: copy` to run in a copied workspace under `../takt-workspaces/{timestamp}-{slug}` without requiring a Git repository or initial commit. Copy workspaces exclude Git metadata, TAKT run/worktree outputs, `node_modules`, and common build caches; their path is recorded as `copy_workspace_path`, and Git-backed branch/PR options fail fast instead of treating the copy as mergeable.
- PR-derived tasks now start with fix-focused triage sections (#834). `takt add --pr`, CLI `--pr`, and pipeline `--pr` order files surface current fix requirements, current-code recheck items, triage notes, and reference context before archived review material so agents do not treat resolved or generated comments as live implementation requirements.
...
- Cursor prompt file mode (#783). `provider_options.cursor.use_prompt_file` lets Cursor provider calls write the full prompt to a temporary `.takt/tmp/cursor-prompts` file and pass only a short file-reference instruction through argv. The default remains full prompt argv, and `TAKT_PROVIDER_OPTIONS_CURSOR_USE_PROMPT_FILE=true` enables the opt-in from env.
- Parallel reviewer report ground-check / RECHECK flow (#821). `provider_options.<provider>.ground_check.enabled` runs groundedness verification after the report phase. `[GROUND_CHECK:NEED_RECHECK]` or invalid tags send the evidence bundle and ground-check report back to the original reviewer once for report regeneration. Ground-check artifacts are written as `<report>.ground-check.md` with normal report history versioning.
- Configurable user-facing timestamp timezone (#769). Set `timezone` in global or project config, or `TAKT_TIMEZONE`, to format generated run directory names and `takt list` timestamps in an IANA time zone or `local`; stored ISO timestamps remain UTC.
- Programmatic faceted prompt payload adapter (#510). `composeTaktPromptPayload()` wraps `faceted-prompting.composePromptPayload()` so TAKT callers can retrieve `systemPrompt`, `userPrompt`, and grouped `copyFiles` directly while using path-based refs for TAKT builtin facets.
...
- Observability now emits estimated token cost counters (`takt.token.estimated_cost_usd`) for priced direct OpenAI/Codex and Anthropic/Claude model usage (#869).
- Observability now emits provider error, quality gate result, loop detection, and cycle detection counters (`takt.provider.errors`, `takt.quality_gate.results`, `takt.workflow.loops_detected`, and `takt.workflow.cycles_detected`) for workflow operations (#869).
...
- Worktree creation now preflights Git repository state before cloning (#439). Non-Git directories and repositories without an initial commit fall back to current-directory execution with a clear hint instead of failing with a misleading missing `main` branch error.
