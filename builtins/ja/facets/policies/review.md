# レビューポリシー

全レビュアーが共有する判断基準と行動原則を定義する。

## 前提

このレビューは、依頼に基づきレビュー対象として提供されたコードベースへ行う防御的な品質・セキュリティ監査である。脆弱性や欠陥の再現・検証はすべて対象リポジトリ内の自己テストとして行い、第三者のシステムへの適用や攻撃手法の開発は目的に含まれない。

## 原則

| 原則 | 基準 |
|------|------|
| 即座修正 | 軽微でも「次のタスク」にしない。今修正できる問題は今修正させる |
| 曖昧さ排除 | 「もう少し整理して」等の曖昧な指摘は禁止。ファイル・行・修正案を具体的に示す |
| ファクトチェック | 推測ではなく実コードを確認してから指摘する |
| 実践的修正案 | 理想論ではなく実装可能な対策を提示する |
| 状態整合性 | 副作用や状態変更は、正常・失敗・中断の各経路で漏れ・重複・不整合がないことを確認する |
| 契約全入口 | 新しい契約は通常入口だけでなく、派生条件・検証・評価・出力・再注入経路まで同じ基準で確認する |
| 契約一貫性 | 共通化・抽象化で担う契約は、既存の同種分岐にも同じ基準で適用されていることを確認する |
| 外部契約検証 | 外部サービス・SDK・生成物の意味契約を、一次情報または実型で確認する |
| 仕様完全性 | 利用者向け契約を変える場合、実装・テスト・ドキュメントが同じ探索順、上書き規則、特殊記法、失敗条件を説明していることを確認する |
| 元要件固定 | タスク指示書の必須項目を、実装都合で optional・対象外・別要件に読み替えない |
| 解消判定 | resolved 判定は修正内容ではなく、元 finding の受入条件と元要件に戻って確認する |
| 振る舞い保証 | テストやログの有無ではなく、期待する振る舞いが検証されているかを確認する |
| ボーイスカウト | 変更箇所と、その正しさ・契約・配線に直接関係する箇所の問題をタスクスコープ内で改善させる |

## スコープ判定

| 状況 | 判定 | 対応 |
|------|------|------|
| 今回の変更で導入された問題 | ブロッキング | REJECT |
| 今回の変更により未使用になったコード（引数、import、変数、関数） | ブロッキング | REJECT（変更起因の問題） |
| 変更箇所・関係箇所の既存問題 | ブロッキング | REJECT（ボーイスカウトルール） |
| 変更の正しさに直接影響する構造的問題 | ブロッキング | スコープ内なら REJECT |
| 変更外ファイルの問題 | 非ブロッキング | 記録のみ（参考情報） |
| 同じ変更ファイル内にあるだけで変更の正しさに直接関係しない既存問題 | 非ブロッキング | 記録のみ（参考情報） |
| タスクスコープを大きく逸脱するリファクタリング | 非ブロッキング | 提案として記載 |

## 判定基準

### REJECT（差し戻し）

以下のいずれかに該当する場合、例外なく REJECT する。

- テストがない新しい振る舞い
- 境界変更（権限、拒否、外部実行、共有状態、状態遷移）に対して、主要な許可/拒否・成功/失敗・隔離/解放の検証がない
- バグ修正にリグレッションテストがない
- `any` 型の使用
- フォールバック値の乱用（`?? 'unknown'`）
- 説明コメント（What/How のコメント）
- 未使用コード（「念のため」のコード）
- オブジェクト/配列の直接変更
- エラーの握りつぶし（空の catch）
- Issue番号・外部制約・除去条件のない TODO/FIXME
- 本質的に同じロジックの重複（DRY違反）
- 同じことをするメソッドの増殖（構成の違いで吸収すべき）
- 特定実装の汎用層への漏洩（汎用層に特定実装のインポート・分岐がある）
- 内部実装のパブリック API エクスポート（インフラ層の関数・内部クラスが公開されている）
- リファクタリングで置き換えられた旧コード・旧エクスポートの残存
- 関連フィールドのクロスバリデーション欠如（意味的に結合した設定値の不変条件が未検証）
- 契約変更後の呼び出し元・生成元・利用元・検証元・テストデータ・派生入口の更新漏れ
- 設定・CLI・ファイル形式などの利用者向け契約変更に対して、ドキュメントや例が優先順位、first-match/merge、inline上書き、scope/特殊参照、失敗条件のいずれかを欠落している
- 共通ヘルパー、正規化関数、builder、adapter の追加・変更後に、同じ契約を持つ既存分岐が旧実装のまま残っている
- タスクで要求されたフィールド、属性、出力、設定、識別子を、明示根拠なく optional・未設定・対象外として扱う、または入口や実行モードによって欠落させている
- 外部サービス・SDK・生成物の操作ごとのエラー型、ステータス、戻り値、冪等性を確認せず、別操作の契約やモックの成功で代用している
- 副作用や状態変更の実行経路に、漏れ・重複・順序不整合がある
- ログ、エラーレスポンス、テスト出力への機密情報露出

DRY違反の修正案は、単に共通化を求めるだけでは不十分とする。以下を満たさない共通化案は不適切であり、別案を検討する。

- 共通化先が既存の責務境界・依存方向と整合している
- 新規公開API・wrapper・helperの追加が、既存契約を不自然に広げていない
- 指示書や plan にない追加抽象化を要求する場合、その必要性が根拠付きで説明されている

### Warning（警告）

ブロッキングではないが改善を推奨する。

- エッジケース・境界値のテスト不足
- テストが実装の詳細に依存
- 関数/ファイルが複雑すぎる
- 命名が実体と乖離している
- Issue番号・外部制約・除去条件がある TODO/FIXME
- 理由なしの `@ts-ignore`、`eslint-disable`

### APPROVE（承認）

全ての REJECT 基準をクリアし、品質基準を満たしている場合に承認する。「条件付き承認」はしない。問題があれば差し戻す。

## 振る舞い証跡の判定

設定値、ログ、スナップショット、最後の状態だけの確認は補助証跡であり、拒否・許可・隔離・解放などの主要な振る舞いを保証する根拠にはならない。

| 証跡 | 判定 |
|------|------|
| 期待する振る舞いが実行結果として観測されている | OK |
| deterministic なテストで主要な境界条件を検証している | OK |
| 外部環境依存のE2Eのみで、主要境界の再現可能な検証がない | 警告またはREJECT |
| 設定値・ログ・スナップショットのみで振る舞いを承認している | REJECT |

## ファクトチェック

指摘する前に必ず事実を確認する。

| やるべきこと | やってはいけないこと |
|-------------|-------------------|
| ファイルを開いて実コードを確認 | 「修正済みのはず」と思い込む |
| 呼び出し元・使用箇所を検索 | 記憶に基づいて指摘する |
| 型定義・スキーマを突合 | 推測でデッドコードと判断する |
| 外部SDK・APIの操作別契約を公式仕様または実型で確認 | コンパイル成功やモック成功だけで意味契約を確認済みとする |
| 生成ファイル（レポート等）とソースを区別 | 生成ファイルをソースコードとしてレビュー |
| ツール出力が正常に読めることを確認 | 文字化け・異常文字を含む出力に基づいて指摘する |
| 「存在しない」主張は対象行を直接読んで検証 | 検索結果だけで「コードにない」と断定する |

### ツール出力の信頼性

ツール出力が正常に読めない場合、読める方法で読み直してから判断する。

| 状況 | 対応 |
|------|------|
| 出力が文字化け・エンコーディング異常を含む | 文字化けを認識し、別の手段（ファイルを直接開く、行番号指定で該当箇所を読む）で正しい内容を取得してから判断する |
| 検索コマンドで該当コードが見つからなかった | ファイルの該当行を直接読み、存在しないことを確認してから指摘する。検索失敗＝コード不在ではない |
| 前回と同じ指摘を出す際に実コードを再確認していない | persists 判定前に必ず現在のコードを読む。前回の記憶で再指摘しない |

## 具体的な指摘の書き方

全ての指摘には以下を含める。

- **どのファイルの何行目か**
- **何が問題か**
- **どう修正すべきか**
- **抽象化や共通化を求める場合は、なぜその配置先が自然か**

```
❌ 「構造を見直してください」
❌ 「もう少し整理してください」
❌ 「リファクタリングが必要です」

✅ 「src/auth/service.ts:45 — validateUser() が3箇所で重複。
     共通関数に抽出してください」
```

## 指摘ID管理（finding_id）

同じ指摘の堂々巡りを防ぐため、指摘をIDで追跡する。

Finding Contract は individual finding 単位ではなく review workflow 全体に適用される。
Finding Contract workflow かどうかは、workflow レベルの `finding_contract` 設定が
宣言されている場合に限って判定する。`findings-ledger.json`、instruction template 内の
専用 `Finding Contract` セクション、output contract の `Observed Findings` table は、
すでに Finding Contract が設定された workflow 内での補助証跡であり、それ自体では
Finding Contract を有効化しない。

Finding Contract 利用時、レビュワーは新規の最終 `finding_id` を採番せず、
`new` / `persists` / `resolved` / `reopened` の lifecycle 分類も行わない。
観測した問題は `Observed Findings` table に raw finding として報告する。既存 ID への
言及は ledger に載っている指摘を参照する場合に限る。ID 採番と lifecycle 対応づけは
findings-manager とエンジンの責務である。

Finding Contract が設定された workflow で parse 可能な ledger がある場合、tracked findings
の正本は ledger である。個別レポートと raw finding 詳細は補助証跡として扱う。ledger が
存在するが不完全な場合は、mapped findings は ledger に従い、unmapped raw findings は
findings-manager reconciliation 待ちの potential new entries として扱う。Finding Contract
が設定された workflow で parse 可能な ledger がない場合、レポート履歴は observed raw
findings の補助証跡としてのみ使う。最終 `finding_id` や lifecycle 状態は割り当てず、
従来ルールも適用しない。ledger 再生成または findings-manager reconciliation を待つ。

### 従来の Finding ID ルール（Finding Contract を使わない workflow 向け）

workflow レベルの `finding_contract` 設定がない場合は、以下の従来ルールに従う。

- REJECT時に挙げる各問題には `finding_id` を必須で付ける
- 同じ問題を再指摘する場合は、同じ `finding_id` を再利用する
- 再指摘時は状態を `persists` とし、未解決である根拠（ファイル/行）を必ず示す
- 新規指摘は状態 `new` とする
- 解消済みは状態 `resolved` として一覧化する
- `finding_id` のない指摘は無効（判定根拠として扱わない）。この legacy 基準は Finding Contract workflow には適用しない
- REJECTは `new` または `persists` の問題が1件以上ある場合のみ許可する
- 前回指摘を解消済みとする場合、別の構造問題や契約悪化を新たに導入していないか確認する

## 再オープン条件（resolved → open）

解消済み指摘を再オープンする場合は、再現可能な根拠を必須とする。

- 前回 `resolved` の指摘を再オープンする場合、以下3点を必須で提示する  
  1. 再現手順（コマンド/入力）  
  2. 期待結果と実結果  
  3. 失敗箇所のファイル/行
- 上記3点が欠ける再オープンは無効（REJECT根拠に使わない）
- 再現手順が変わる場合は別問題として新規 `finding_id` を発行する

## finding_id の意味固定

同じ ID に別問題を混在させない。

- 同一 `finding_id` は同一問題にのみ使用する
- 問題の意味・根拠ファイル・再現条件が変わる場合は新規 `finding_id` を発行する
- 同一 `finding_id` の説明を後から別問題に差し替えることを禁止する

## テストファイルの扱い

テストファイルの長さや重複は、原則として保守性の警告として扱う。

- テストファイルの行数超過・重複コードは原則 `Warning`
- 以下の実害が再現できる場合のみ `REJECT` 可能  
  - テスト不安定化（フレーク）  
  - 誤検知/検知漏れ  
  - 回帰検出不能
- 「長すぎる」「重複がある」だけでは `REJECT` しない

## 変更履歴ファイルの扱い

過去時点の事実を記録するファイル/セクション (例: `CHANGELOG.md`, `RELEASE_NOTES.md`, `MIGRATION.md`) は、現行コードの仕様書ではなく履歴である。これらを評価する際は履歴としての正しさを基準とする。

| 対象 | 判定 |
|------|------|
| 過去エントリの設定キー・API名・挙動が現行コードと一致しない | REJECT 禁止 |
| 過去リリース時点では正しかった記述 | 修正要求禁止 |
| 新規追加エントリの対象リリース時点での事実誤認 | REJECT 可 |
| Markdown 形式崩れ・重複・リンク切れ・明らかな表記ミス | REJECT または Warning 可 |

### 判断基準

- 履歴は「現在どう動くか」ではなく「その時点で何が変更されたか」を記録する
- 現行コードで名称変更・仕様変更されていても、過去エントリを書き換える理由にはならない
- 過去エントリを修正対象にする場合は、対象リリース時点でも誤りだった根拠を示す
- ファイル名 (`CHANGELOG.md` 等) や慣用的な見出し (`### Changed`, `### Added`, リリース日付つき見出し) で「履歴である」と判別する
- 現行スキーマや現行設定キーとの不一致だけを根拠に履歴ファイル/セクションを REJECT してはならない

## ボーイスカウトルール

来たときよりも美しく。

### 対象

- 変更箇所、および変更の正しさ・契約・配線に直接影響する関係箇所の既存問題（未使用コード、不適切な命名、壊れた抽象化）
- 変更の正しさに直接影響する構造的な問題（責務の混在、不要な依存）

### 対象外

- 変更していないファイル（既存問題として記録のみ）
- 同じ変更ファイル内にあるだけで、変更の正しさ・契約・配線に直接影響しない既存問題
- タスクスコープを大きく逸脱するリファクタリング（提案として記載、非ブロッキング）

### 判定

| 状況 | 判定 |
|------|------|
| 変更箇所・関係箇所に明らかな問題がある | REJECT — 一緒に修正させる |
| 冗長な式（同値の短い書き方がある） | REJECT |
| 不要な分岐・条件（到達しない、または常に同じ結果） | REJECT |
| 数秒〜数分で修正可能な問題 | REJECT（「非ブロッキング」にしない） |
| 変更の結果として未使用になったコード（引数・import等） | REJECT — 変更起因であり「既存問題」ではない |
| 修正にリファクタリングが必要（スコープが大きい） | 記録のみ（技術的負債） |

既存コードの踏襲を理由にした問題の放置は認めない。既存コードが悪い場合、それに合わせるのではなく改善する。

## 判定ルール

- 変更箇所、および変更の正しさ・契約・配線に直接影響する関係箇所で検出した問題は、既存コードであってもブロッキング（REJECT対象）として扱う
- 「既存問題」「非ブロッキング」に分類してよいのは、変更と直接関係しない箇所の問題のみ
- 「コード自体は以前から存在していた」は、変更箇所・関係箇所の問題を非ブロッキングにする理由にならない
- 問題が1件でもあればREJECT。「APPROVE + 警告」「APPROVE + 提案」は禁止

## レビューの基本手順

全レビュアーが共通して踏むべき手順を定義する。各 instruction で個別に書かない。

### 差分の基準点（ベースを固定する）

レビュー対象は、タスク開始時点（ベース）からの累積差分全体とする。直前イテレーションの修正分だけを対象にしない。

- fix ↔ review のループでは、毎回ベースから差分を取り直して全体を評価する。基準点を直近の修正へ動かさない
- ベースは統合先ブランチとのマージベース、または `plan` / `order` に記録された起点とする。`Previous Response` の「変更内容」だけを差分とみなさない
- 過去イテレーションで紛れ込んだ要求外の変更（無関係なコメント削除、改名、整形、契約変更、テスト緩和）は、最新の修正レポートに現れなくても累積差分には残る。毎回これを照合する
- finding の状態追跡（new / persists / resolved）は基準点を固定した前提で行う。差分基準を狭めて「もう差分に無い」と判断しない

### 一次情報の参照

- `order.md`、`plan.md`、実コードを一次情報として参照する
- 過去ステップの判断（前段レビュー結果、計画上の決定）は補助情報として扱う
- 情報が競合する場合は、`order.md` / `plan.md` / 実コードを優先する
- 利用者向け仕様を変えた場合、ドキュメントと設定例も契約の一部として扱い、要件に列挙された挙動が欠落していないか確認する

### 設計判断の参照

- 実装ステップが `coder-decisions.md` を出力している場合は確認し、記録された設計判断を把握する
- 記録された意図的な判断は、それだけを理由に false positive として退けない。`order.md` / `plan.md` / 実コードと照らして妥当性を評価する
- 設計判断自体に問題があれば指摘する

### 契約追加・変更の全入口検証

差分で新しい契約（設定値、状態、条件式、ファイル形式、イベント、builder、adapter、状態遷移関数）を追加・変更した場合、その契約が通る入口・出口・再注入経路を列挙して照合する。

- 定義、生成、正規化、検証、評価、永続化、出力、イベント発行の各経路に同じ契約が適用されているか確認する
- 通常入口だけでなく、派生条件、集約条件、親子ワークフロー、ループ判定、早期終了、例外経路を確認する
- 保存済みデータや外部由来データを JSON、Markdown、ログ、イベント、次の指示へ再注入する場合、エスケープ・境界・失敗時の扱いを契約に含める
- 同じ責務を持つ既存の return、throw、catch、early return、分岐、呼び出し元を検索する
- 既存分岐が新しい契約を満たさない場合は、コードが以前から存在していても変更の関係箇所として扱う
- 新しい経路だけにテストが偏り、既存の同種分岐や派生入口で契約が検証されていない場合はカバレッジ不足として扱う
- タスクで要求された契約を optional・対象外・別要件にする場合は、タスク指示書、仕様、明示的なユーザー指示の根拠を必須とする
- 「タスク要件に明記されていない」ことは、差分で導入した契約不整合を非ブロッキングにする理由にならない

### 副作用と状態遷移の確認

変更が外部呼び出し、設定反映、セッション、キュー、ロック、購読、キャッシュ、一時資源などの副作用や状態変更を伴う場合は、正常系だけで判断しない。

- 入口、正常終了、早期 return、例外、再試行、中断、後片付けの経路を追う
- 取得・開始・登録・反映したものが、対応する経路で過不足なく扱われることを確認する
- 同じ副作用が重複して実行されないこと、必要な副作用が失敗経路で抜けないことを確認する
- 共有状態や後続処理に影響する変更では、途中失敗後の状態が次の実行を壊さないことを確認する
- これらの確認ができていない場合は、動作確認済みとして扱わない

### 前回指摘の追跡

**優先順位:**

1. Finding Contract が設定された workflow で parse 可能な Finding Contract ledger / `findings-ledger.json` が利用できる場合、tracked findings の正本として ledger を使用する。修正対象は ledger 上の open findings（`new`、`persists`、`reopened`）のみとし、resolved / closed findings は修正対象外とする。個別レポートは ledger から辿る補助証跡として扱う。
2. ledger が存在するが不完全な場合は、mapped findings は ledger に従い、unmapped raw findings は findings-manager reconciliation 待ちの potential new entries として扱う。
3. Finding Contract が設定された workflow で parse 可能な ledger がない場合、Report Directory の最新レビューは observed raw findings の補助証跡としてのみ使う。最終 `finding_id` や lifecycle 状態は割り当てず、従来ルールも適用しない。ledger 再生成または findings-manager reconciliation を待つ。
4. `finding_contract` 設定がない workflow では、Report Directory の最新レビューを primary evidence として扱い、従来ルールを適用する:
   - Report Directory 内で、このステップが前回までに出力したレビューレポートとそのタイムスタンプ付き履歴を確認する
   - 無印ファイルを最新結果、直前のタイムスタンプ付きファイル（`{レポート名}.{タイムスタンプ}`）を前回結果として扱う
   - `Previous Response` がある場合は補助情報として参照してよいが、findings の状態判定はレポート履歴を優先する
   - 前回レポートにある open findings を、今回のレポートへ欠落させない
   - 各 finding の状態判定（`new` / `persists` / `resolved` / `reopened`）は finding_id 管理ルールに従う
   - `resolved` は修正差分の有無ではなく、元 finding の期待結果と元要件を満たす場合のみ認める

### 判定の最終手順

1. 検出した問題ごとに、スコープ判定表と判定ルールに基づいてブロッキング/非ブロッキングを分類する
2. テスト・ビルド・動作確認を根拠に書く場合は、確認対象・確認内容・結果をレポート内に具体的に残す
3. ブロッキング問題（`new`、`persists`、または `reopened`）が 1 件でもあれば REJECT と判定する

## 堂々巡りの検出

同じ種類の指摘が繰り返される場合、細かい修正指示の反復ではなくアプローチ自体を見直す。
