...
| `isolation` | `copy`、`worktree`、`none`（明示的な隔離方式） |
| `worktree` | `true`（自動）、パス文字列、または省略（カレントディレクトリで実行） |
| `copy_workspace_path` | `isolation: copy` 実行後に記録されるコピー先パス |
...
Git repository でないディレクトリ、または初回 commit 前の repository では、`isolation: copy` を使うと現在のディレクトリを `../takt-workspaces/{timestamp}-{slug}` にコピーしてから workflow を実行できます。

```yaml
tasks:
  - name: local-copy-task
    status: pending
    task: ローカルの試作コードを整理する
    workflow: default
    isolation: copy
```

copy workspace はディレクトリコピーであり、Git branch として merge できる成果物ではありません。そのため `branch`、`auto_pr`、`draft_pr`、`managed_pr`、`should_publish_branch_to_origin` など Git 前提の設定は `copy` では拒否されます。実行結果の場所は task record の `copy_workspace_path` に保存されます。

初期実装では、巨大化や再帰を避けるため `.git/`、`.takt/runs/`、`.takt/worktrees/`、`node_modules/`、`dist/`、`build/`、`coverage/` などをコピー対象から除外します。`.takt/facets/` など workflow 実行に必要な project-local 設定はコピーされます。
