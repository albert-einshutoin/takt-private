...
timezone: Asia/Tokyo          # 表示用 timestamp と run 名生成の timezone（IANA ID または 'local'。未設定時は UTC）
logging:
  level: info                 # ログレベル: debug, info, warn, error
provider: claude              # デフォルト provider: claude, claude-sdk, claude-terminal, codex, codex-cli, opencode, opencode-cli, cursor, cursor-cli, copilot, kiro, agy-cli, または mock
...
# サブスク/ログイン済みセッション専用実行（省略可）
# 有効化すると、API key 設定と allowed_providers 外の provider を拒否します。
...
# API キー設定（省略可）
# subscription_only が true の場合は設定しないでください。
...
| `timezone` | IANA timezone ID \| `"local"` | 未設定時は UTC | 表示用 timestamp と生成される run directory 名に使う timezone。保存される ISO timestamp は UTC のままです。`TAKT_TIMEZONE` でも設定できます。 |
| `logging.level` | `"debug"` \| `"info"` \| `"warn"` \| `"error"` | `"info"` | ログレベル |
| `provider` | `"claude"` \| `"claude-sdk"` \| `"claude-terminal"` \| `"codex"` \| `"codex-cli"` \| `"opencode"` \| `"opencode-cli"` \| `"cursor"` \| `"cursor-cli"` \| `"copilot"` \| `"kiro"` \| `"agy-cli"` \| `"mock"` | `"claude"` | デフォルト AI provider（`claude` = ヘッドレス CLI モード、`claude-sdk` = SDK/API モード、`*-cli` = 外部 CLI のログイン/セッション利用モード） |
| `logging.trace` | boolean | `false` | trace レベルのログを有効化（高頻度のデバッグノイズを抑制） |
| `model` | string | - | デフォルトモデル名（provider にそのまま渡される） |
| `subscription_only` | boolean | `false` | API key 設定、allowlist 外の provider、workflow 内 provider 上書き、実行時 provider 上書きを subscription-only allowlist で拒否 |
| `allowed_providers` | provider[] | `codex-cli`, `cursor-cli`, `opencode-cli`, `agy-cli`, `mock` | `subscription_only: true` のときだけ使う allowlist。指定できるのは subscription-safe provider のみ。TAKT 管理の `opencode_api_key` 設定を使わず、OpenCode Go/Zen など OpenCode 側の credential store を使う場合だけ `opencode` を明示追加できます |
| `forbidden_providers` | string[] | `[]` | `subscription_only: true` のときに追加で拒否する provider 名。`codex`, `opencode`, `claude-sdk` など組織ポリシーの明示に使えます |
...
timezone: Asia/Tokyo          # 表示用 timestamp と run 名生成の timezone を上書き
logging:
...
| フィールド | 型 | デフォルト | 説明 |
|-----------|------|---------|------|
| `provider` | `"claude"` \| `"claude-sdk"` \| `"claude-terminal"` \| `"codex"` \| `"codex-cli"` \| `"opencode"` \| `"opencode-cli"` \| `"cursor"` \| `"cursor-cli"` \| `"copilot"` \| `"kiro"` \| `"agy-cli"` \| `"mock"` | - | provider 上書き |
| `model` | string | - | モデル名の上書き（provider にそのまま渡される） |
| `subscription_only` | boolean | グローバル設定値 | このプロジェクトで API key 設定と subscription-only allowlist 外の provider を拒否 |
| `allowed_providers` | provider[] | グローバル設定値またはビルトイン allowlist | プロジェクトレベルの subscription-only provider allowlist |
| `forbidden_providers` | string[] | グローバル設定値または `[]` | プロジェクトレベルで追加拒否する provider 名 |
| `timezone` | IANA timezone ID \| `"local"` | グローバル設定値または未設定時は UTC | 表示用 timestamp と生成される run directory 名に使う timezone のプロジェクト上書き。保存される ISO timestamp は UTC のままです。`TAKT_TIMEZONE` でも設定できます。 |
| `allow_git_hooks` | boolean | `false` | TAKT 管理の auto-commit 時に git hooks を許可 |
| `allow_git_filters` | boolean | `false` | TAKT 管理の auto-commit 時に git filter を許可 |
| `auto_pr` | boolean | - | worktree 実行後に PR を自動作成 |
...
TAKT は Claude、Codex、OpenCode、Cursor、Copilot、Kiro、Antigravity 系 CLI provider をサポートしています。`codex` や `opencode` のような SDK/API provider は API キーを使えます。`codex-cli`、`opencode-cli`、`cursor-cli`、`agy-cli` のような CLI-only provider はログイン済みローカル CLI セッションでの利用を想定しており、TAKT は起動前に一般的な API key 環境変数を取り除きます。

`subscription_only: true` を有効にしている場合、API key config フィールドと allowlist 外の provider は workflow 実行前に拒否されます。通常は `codex-cli`, `cursor-cli`, `opencode-cli`, `agy-cli` を使ってください。OpenCode Go/Zen など OpenCode 側の credential store を使う場合は `opencode` を明示的に allowlist へ追加できますが、subscription-only mode では引き続き `opencode_api_key` と `TAKT_OPENCODE_API_KEY` は拒否されます。

長い run の前に `devloopd doctor --subscription-only` を実行すると、必須 CLI、GitHub 認証、禁止された API key 環境変数、TAKT config、project workflow の provider 参照を確認できます。詳細は [devloopd Guide](./devloopd.ja.md) を参照してください。
...
- CLI-only provider（`codex-cli`、`opencode-cli`、`cursor-cli`、`agy-cli`）は subprocess 起動前に一般的な API key 環境変数を削除し、SDK/API provider へ意図的にフォールバックしません。
...
provider option の leaf は環境変数でも上書きできます。OpenCode の model variant は `TAKT_PROVIDER_OPTIONS_OPENCODE_VARIANT=high` で `provider_options.opencode.variant` を設定できます。provider base URL は `TAKT_PROVIDER_OPTIONS_CODEX_BASE_URL=http://127.0.0.1:8787/v1` または `TAKT_PROVIDER_OPTIONS_CLAUDE_BASE_URL=http://127.0.0.1:8787` を使用できます。これらは config layer を設定するもので、step や workflow routing の `base_url` leaf は上書きしません。Claude terminal は `TAKT_PROVIDER_OPTIONS_CLAUDE_TERMINAL_BACKEND=tmux`、`TAKT_PROVIDER_OPTIONS_CLAUDE_TERMINAL_TIMEOUT_MS=900000`、`TAKT_PROVIDER_OPTIONS_CLAUDE_TERMINAL_KEEP_SESSION=false`、`TAKT_PROVIDER_OPTIONS_CLAUDE_TERMINAL_TRANSCRIPT_POLL_INTERVAL_MS=500` を使用できます。Cursor prompt file mode は `TAKT_PROVIDER_OPTIONS_CURSOR_USE_PROMPT_FILE=true` で `provider_options.cursor.use_prompt_file` を設定できます。Kiro の custom agent は `TAKT_PROVIDER_OPTIONS_KIRO_AGENT=planner-agent` で `provider_options.kiro.agent` を設定できます。Antigravity の print mode timeout は `TAKT_PROVIDER_OPTIONS_AGY_PRINT_TIMEOUT=30s` で `provider_options.agy.print_timeout` を設定できます。
...
parallel reviewer の report phase 後に、別 provider で report の根拠確認を行うには `provider_options.<provider>.ground_check` を使います。`ground_check` は orchestration 用の設定であり、実際の provider SDK call へは渡されません。
...
`provider`、`model`、`provider_options` を省略すると、元 reviewer step の解決済み provider / model / providerOptions を使います。`enabled: false` は、上位 layer で有効化された ground-check をその layer で無効化します。優先順位は通常の provider option leaf と同じです。step `provider_options` > `provider_routing.steps` > `provider_routing.tags` > `provider_routing.personas` > deprecated の `persona_providers` > `workflow_config.provider_options` > project `.takt/config.yaml` > global `~/.takt/config.yaml` の順に解決されます。

ground-check の decision tag は `[GROUND_CHECK:VALID]` または `[GROUND_CHECK:NEED_RECHECK]` のどちらか 1 つだけです。tag がない、複数ある、未知の tag がある場合は fail closed として `NEED_RECHECK` になります。artifact は `<report>.ground-check.md` に保存されます。例: `security-review.md` に対して `security-review.ground-check.md`。
...
Cursor はデフォルトで CLI argv からタスク指示を受け取ります。非常に長い prompt は shell や OS のコマンドライン長制限に当たることがあります。Cursor prompt file mode を有効にすると、full prompt を一時ファイルへ書き、`cursor-agent` には短いファイル参照 instruction だけを渡します。
...
有効時、TAKT は system prompt と task prompt を結合した full prompt を `.takt/tmp/cursor-prompts` 配下へ書き、Cursor にそのファイルを読むよう指示し、provider call の成功・失敗後にファイルを削除します。デフォルトは `false` のため、既存の Cursor 連携は従来どおり full prompt を argv で受け取ります。
