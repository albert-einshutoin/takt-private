...
| `subscription_only` | boolean | `false` | API key 設定、SDK/API provider、workflow 内 provider 上書き、実行時 provider 上書きを subscription-only allowlist で拒否 |
| `allowed_providers` | provider[] | `codex-cli`, `cursor-cli`, `opencode-cli`, `agy-cli`, `mock` | `subscription_only: true` のときだけ使う allowlist。指定できるのは subscription-safe provider のみ |
...
`subscription_only: true` を有効にしている場合、API key config フィールドと SDK/API provider は workflow 実行前に拒否されます。代わりに `codex-cli`, `cursor-cli`, `opencode-cli`, `agy-cli` を使ってください。
