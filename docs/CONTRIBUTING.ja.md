...
ソース開発の標準環境は **Node.js 22.13.1** と **npm 10.9.2** です。
`packageManager` は Corepack で再現可能な推奨ペアとしてこのバージョンを固定します。
開発環境として許容する範囲は Node.js **20 系の 20.19 以降、22 系の 22.13 以降、
または 24 以降**、npm **10 系の 10.9.2 以降**です。これにより npm 11 は許可せず、
固定済み Nix flake が選ぶ Node/npm のパッチリリースを利用できます。
公開パッケージは引き続き Node.js **20.6.0 以降**をサポートします。新しい開発基準は
lint などの開発ツールが必要とするもので、TAKT 製品ランタイムの最低要件ではありません。
