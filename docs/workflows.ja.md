...
instruction facet では `facets/partials/instructions/<name>.md` に置いた共有 Markdown partial を include できます。partial が存在するときだけ展開したい場合は `{review-common}`、見つからない場合に workflow load を失敗させたい場合は `{partial:review-common}` を使います。partial は package-local、project、global、builtin layer の順で解決され、partial 内から別 partial を include できます。循環参照は拒否されます。`{task}`、`{previous_response}`、`{report_dir}` などの builtin template 変数は、その後に instruction builder が従来どおり展開します。

`quality_gates` の文字列は従来どおり agent step の AI への完了条件としてプロンプトに含まれます。`type: command` の gate は agent step 完了後に worktree 内で実行され、終了コード `0` の場合のみ成功します。workflow YAML の command gate を使うには config 側で `workflow_command_gates.custom_scripts: true` を有効にする必要があります。出力キャプチャには上限があり、上限超過時はフィードバック用の stdout / stderr を切り詰めますが、終了コード `0` の command を失敗扱いにしたり停止したりはしません。失敗時は command のメタデータ、cwd、終了コードまたは timeout 情報、出力切り詰め情報、output log path、上限付きでサニタイズされた stdout / stderr が同じ agent step の差し戻し入力に含まれます。キャプチャ済みの raw stdout / stderr はローカルの output log にも保存されます。`system` と `workflow_call` step では `quality_gates` を指定できません。
...
| `{partial:name}` | `facets/partials/instructions/name.md` を厳密に include する。見つからない場合は workflow load に失敗する |
...
| `provider_options.agy.print_timeout` | - | Antigravity CLI の print mode timeout。`agy --print-timeout` として渡される |
