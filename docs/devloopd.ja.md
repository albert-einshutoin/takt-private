...
product-policy escalation criteria、expected changed surfaces が入ります。issue scout は
open issue、open PR、branch name、過去の ledger decision と dedupe し、残った candidate を
...
`devloopd merge-if-safe` は機械的な merge 実行者です。LLM output だけで PR を merge しません。このコマンドは `gh pr view` で PR metadata、`gh pr diff --name-only` で変更ファイル、`gh pr checks` で GitHub checks を確認し、通過した場合だけ auto-merge を有効化します。
