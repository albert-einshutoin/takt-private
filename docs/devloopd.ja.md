...
`devloopd schedule-template` は、personal automation 用の repo-local な launchd
と cron snippet を出力します。生成される command は automation work の前に
`npm run check:personal` を通し、stale state recovery を実行し、その後
`--max-cycles 1 --safety-profile safe-default` の bounded staged loop を起動します。
...
通常出力には install、status、uninstall、dry-run のコマンドが含まれます。
macOS launchd では、生成した repo-local plist から install します。
...
cron では、label marker 付きの snippet を install / remove します。
...
install 前に、出力された dry-run command を実行してください。launchd と cron は
interactive shell startup file を安定して読み込まないため、template 側で明示的な
`PATH`、`TAKT_LOOP_GH_TIMEOUT_MS`、`TAKT_LOOP_SAFETY_PROFILE` を設定します。
生成した snippet と log は、Git から無視される `.devloop/` 配下に置いてください。
