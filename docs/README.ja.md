...
TAKT は、AI コーディングエージェントを再現可能な開発ワークフローとして動かす OSS CLI です。計画、実装、レビュー、修正ループ、人間への確認、権限、出力契約を YAML で定義し、隔離された worktree または copy workspace と追跡可能なログ付きでタスクを実行します。
...
- 積んだタスクを隔離された worktree または copy workspace で実行し、後からログとレポートを確認できる
...
レビューを飛ばせない構造にし、問題があれば修正へ戻し、必要なら人間に判断を戻します。タスクは worktree または copy workspace で隔離され、各 step の結果はログとレポートに残るため、タスクから結果までの流れを後から追跡できます。
...
# 積んだタスクを隔離された worktree または copy workspace で実行します
takt run
...
| 作業がカレントツリーに直接入ることが多い | 積んだタスクは隔離された worktree または Git 不要の copy workspace で実行できる |
...
| [`takt project-template`](./project-templates.ja.md#運用cliとtaktdesk連携) | portableな`.takt` templateのexport・確認・適用・更新・一覧・rollbackを行います |
...
| `devloopd product-policy replay --cwd /path/to/repo` | active な product-policy classifier fixture を replay します |
