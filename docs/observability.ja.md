...
| `takt.token.estimated_cost_usd` | 単価を持つ provider/model 組み合わせの推定 token cost（USD） |
| `takt.provider.errors` | provider、model、step、正規化済み error type ごとの provider error 数 |
| `takt.quality_gate.results` | gate name、gate type、step ごとの quality gate pass/fail 数 |
| `takt.workflow.loops_detected` | 同一 step 連続実行 loop detector の検出数 |
| `takt.workflow.cycles_detected` | loop monitor の cycle threshold 到達数 |
...
推定 cost metric は `src/shared/pricing/tokenCost.ts` の per-million-token 単価表を使い、direct OpenAI/Codex と Anthropic/Claude provider model のみを対象にします。単価未登録の provider や未知の model name では `takt.token.estimated_cost_usd` を送出しません。subscription、proxy、custom router provider で誤った USD 値を出さないためです。Claude の cache creation は TAKT の usage record に cache duration が含まれないため、5 分 cache-write 単価で推定します。

provider error metric は `takt.provider.error_type` で rate limit、provider 側で正規化された failure category、runtime exception を区別します。string の quality gate directive は prompt text を metric attribute に保存せず、`ai_gate_<n>` としてラベル付けします。command gate は明示 `name` があればそれを使い、未指定なら `command_gate_<n>` を使います。
