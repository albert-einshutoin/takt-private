...
- PR 由来 task の先頭に修正焦点の triage section を追加しました (#834)。`takt add --pr`、CLI `--pr`、pipeline `--pr` の order file では、現在の修正要件、最新コードで再確認すべき項目、triage notes、reference context を古い review material より前に出し、resolved comment や生成コメントを現在の実装要件として誤扱いしにくくします。
...
- parallel reviewer report の ground-check / RECHECK flow を追加しました (#821)。`provider_options.<provider>.ground_check.enabled` で report phase 後の根拠確認を有効化でき、`[GROUND_CHECK:NEED_RECHECK]` または不正 tag の場合は元 reviewer に evidence bundle と ground-check report を渡して 1 回だけ report を再生成します。ground-check artifact は `<report>.ground-check.md` に保存され、通常 report と同じ履歴 versioning を使います。
...
- programmatic faceted prompt payload adapter を追加しました (#510)。`composeTaktPromptPayload()` は `faceted-prompting.composePromptPayload()` を wrap し、TAKT caller が TAKT builtin facet の path-based ref を使いながら `systemPrompt`、`userPrompt`、facet kind ごとの `copyFiles` を直接取得できるようにします。
...
- traced-config runtime helper に subprocess timeout を追加しました (#688)。workflow step 遷移時に Node 起動や traced-config 解決が停止した場合、TAKT は helper を終了し、run 全体を無期限に固める代わりに明確な `traced-config helper timed out` エラーを報告します。デフォルトは 10 秒で、`TAKT_TRACED_CONFIG_HELPER_TIMEOUT_MS` で調整できます。
